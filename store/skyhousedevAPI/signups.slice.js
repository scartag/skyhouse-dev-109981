...
export const api_v1_signup_create = createAsyncThunk("signups/api_v1_signup_create", async payload => {
  const response = await apiService.api_v1_signup_create(payload);
  return response.data;
});
export const rest_auth_registration_create = createAsyncThunk("signups/rest_auth_registration_create", async payload => {
...
const signupsSlice = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(api_v1_signup_create.pending, (state, action) => {
